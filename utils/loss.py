...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        BCEcls_each = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device), reduction='none')
        BCEobj_each = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device), reduction='none')
        ...
        if g > 0:
            ...
            BCEcls_each, BCEobj_each = FocalLoss(BCEcls_each, g), FocalLoss(BCEobj_each, g)
...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        if 'obj_balance' in h:
            balance = h['obj_balance']
            if isinstance(balance, str):
                balance = [float(x) for x in balance.replace(',', ' ').split()]
            if len(balance) != det.nl:
                raise ValueError(f"obj_balance length {len(balance)} must match detection layers {det.nl}")
            self.balance = [float(x) for x in balance]
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj = BCEcls, BCEobj
        self.BCEcls_each, self.BCEobj_each = BCEcls_each, BCEobj_each
        self.gr, self.hyp, self.autobalance = model.gr, h, autobalance
        for k in 'na', 'nc', 'nl', 'anchors', 'stride':
            setattr(self, k, getattr(det, k))
        self.small_obj_assign = bool(h.get('small_obj_assign', 0))
        self.small_obj_layer = int(h.get('small_obj_layer', 0))
        self.small_obj_max_px = float(h.get('small_obj_max_px', 64.0))
        self.small_obj_topk = max(1, int(h.get('small_obj_topk', 2)))
        self.small_obj_loss = bool(h.get('small_obj_loss', 0))
        self.small_obj_loss_max_px = float(h.get('small_obj_loss_max_px', self.small_obj_max_px))
        self.small_obj_box_gain = max(1.0, float(h.get('small_obj_box_gain', 1.0)))
        self.small_obj_cls_gain = max(1.0, float(h.get('small_obj_cls_gain', 1.0)))
        self.small_obj_obj_gain = max(1.0, float(h.get('small_obj_obj_gain', 1.0)))
        self.small_obj_aux_gain = max(1.0, float(h.get('small_obj_aux_gain', 1.0)))
        self.small_obj_min_dynamic_k = max(0, int(h.get('small_obj_min_dynamic_k', 0)))

    def target_small_mask(self, targets, imgs):
        if (not self.small_obj_loss and self.small_obj_min_dynamic_k <= 0) or targets.shape[0] == 0:
            return None
        img_px = float(max(imgs.shape[-2:]))
        wh_px = targets[:, 4:6] * img_px
        return wh_px.max(1)[0] <= self.small_obj_loss_max_px

    @staticmethod
    def weighted_mean(loss, weights):
        if weights is None:
            return loss.mean()
        weights = weights.to(loss.dtype)
        return (loss * weights).sum() / weights.sum().clamp_min(1.0)

    def target_weights(self, targets, imgs, gain):
        if not self.small_obj_loss or gain <= 1.0 or targets.shape[0] == 0:
            return None
        small = self.target_small_mask(targets, imgs)
        if small is None or not small.any():
            return None
        weights = torch.ones(targets.shape[0], device=targets.device, dtype=targets.dtype)
        weights[small] = gain
        return weights
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                small_box_w = self.target_weights(targets[i], imgs, self.small_obj_box_gain)
                lbox += self.weighted_mean(1.0 - iou, small_box_w)  # iou loss
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    small_cls_w = self.target_weights(targets[i], imgs, self.small_obj_cls_gain)
                    cls_loss = self.BCEcls_each(ps[:, 5:], t).mean(1)
                    lcls += self.weighted_mean(cls_loss, small_cls_w)  # BCE
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            if self.small_obj_min_dynamic_k > 0:
                small_dynamic = self.target_small_mask(this_target, imgs)
                if small_dynamic is not None and small_dynamic.any():
                    min_k = min(self.small_obj_min_dynamic_k, pair_wise_iou.shape[1])
                    dynamic_ks[small_dynamic] = torch.maximum(
                        dynamic_ks[small_dynamic],
                        torch.full_like(dynamic_ks[small_dynamic], min_k))
        ...
        for i in range(nl):
            if matching_targets[i] != []:
                ...
                matching_bs[i] = torch.empty(0, device=targets.device, dtype=torch.int64)
                matching_as[i] = torch.empty(0, device=targets.device, dtype=torch.int64)
                matching_gjs[i] = torch.empty(0, device=targets.device, dtype=torch.int64)
                matching_gis[i] = torch.empty(0, device=targets.device, dtype=torch.int64)
                matching_targets[i] = torch.empty((0, 6), device=targets.device, dtype=targets.dtype)
                matching_anchs[i] = torch.empty((0, 2), device=targets.device, dtype=targets.dtype)
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ...
                small_box_aux_w = self.target_weights(
                    targets_aux[i], imgs, self.small_obj_box_gain * self.small_obj_aux_gain)
                lbox += 0.25 * self.weighted_mean(1.0 - iou_aux, small_box_aux_w)  # iou loss
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    small_cls_aux_w = self.target_weights(
                        targets_aux[i], imgs, self.small_obj_cls_gain * self.small_obj_aux_gain)
                    cls_loss_aux = self.BCEcls_each(ps_aux[:, 5:], t_aux).mean(1)
                    lcls += 0.25 * self.weighted_mean(cls_loss_aux, small_cls_aux_w)  # BCE

            obj_loss = self.BCEobj_each(pi[..., 4], tobj)
            obj_loss_aux = self.BCEobj_each(pi_aux[..., 4], tobj_aux)
            if self.small_obj_loss and self.small_obj_obj_gain > 1.0:
                obj_w = torch.ones_like(tobj)
                obj_w_aux = torch.ones_like(tobj_aux)
                if n:
                    small_obj_w = self.target_weights(targets[i], imgs, self.small_obj_obj_gain)
                    if small_obj_w is not None:
                        obj_w[b, a, gj, gi] = torch.maximum(obj_w[b, a, gj, gi], small_obj_w.to(obj_w.dtype))
                if n_aux:
                    small_obj_aux_w = self.target_weights(
                        targets_aux[i], imgs, self.small_obj_obj_gain * self.small_obj_aux_gain)
                    if small_obj_aux_w is not None:
                        obj_w_aux[b_aux, a_aux, gj_aux, gi_aux] = torch.maximum(
                            obj_w_aux[b_aux, a_aux, gj_aux, gi_aux], small_obj_aux_w.to(obj_w_aux.dtype))
                obji = (obj_loss * obj_w).mean()
                obji_aux = (obj_loss_aux * obj_w_aux).mean()
            else:
                obji = obj_loss.mean()
                obji_aux = obj_loss_aux.mean()
    ...
    def add_small_target_fallback(self, i, p, targets, gain, anchors, match_mask):
        if not self.small_obj_assign or i != self.small_obj_layer or targets.shape[1] == 0:
            return None
        base = targets[0] * gain
        small_wh_px = base[:, 4:6] * self.stride[i]
        small = small_wh_px.max(1)[0] <= self.small_obj_max_px
        no_standard_match = ~match_mask.any(0)
        small = small & no_standard_match
        if small.sum() == 0:
            return None

        base = base[small]
        wh = base[:, 4:6].clamp(min=1e-3)
        ratios = torch.max(wh[None] / anchors[:, None], anchors[:, None] / wh[None]).max(2)[0]
        k = min(self.small_obj_topk, anchors.shape[0])
        _, anchor_idx = torch.topk(ratios, k=k, dim=0, largest=False)
        t = base.repeat(k, 1, 1)
        t[:, :, 6] = anchor_idx.float()
    ...
    def find_5_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter
                t_small = self.add_small_target_fallback(i, p, targets, gain, anchors, j)
                if t_small is not None:
                    t = torch.cat((t, t_small), 0)
