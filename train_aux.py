...
def train(hyp, opt, device, tb_writer=None):
    ...
    batch_sleep_ms = max(0, int(os.getenv('YOLOV7_BATCH_SLEEP_MS', '0')))
    if batch_sleep_ms and rank in [-1, 0]:
        logger.info(f'Batch sleep throttle: {batch_sleep_ms} ms')
    ...
    # Image sizes
    gs = max(int(getattr(model, 'input_stride', int(model.stride.max()))), 32)  # grid size (input-safe stride)
    ...
    close_mosaic = opt.close_mosaic if opt.close_mosaic >= 0 else int(hyp.get('close_mosaic', 0))
    mosaic_closed = False
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if close_mosaic and not mosaic_closed and epoch >= epochs - close_mosaic:
            hyp['mosaic'] = 0.0
            hyp['mixup'] = 0.0
            hyp['copy_paste'] = 0.0
            hyp['paste_in'] = 0.0
            ...
            mosaic_closed = True
            if rank in [-1, 0]:
                logger.info(f'Closing mosaic/mixup/copy-paste for final {close_mosaic} epochs')
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if distill_loss is not None:
                    ...
                    loss = loss + opt.distill_weight * loss_distill * imgs.shape[0]
                if rank != -1:
        ...
        if rank in [-1, 0]:
            # mAP
            ema.update_attr(model, include=['yaml', 'nc', 'hyp', 'gr', 'names', 'stride', 'input_stride', 'class_weights'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=2, help='global random seed')
    ...
    parser.add_argument('--dist-backend', type=str, default='auto', choices=['auto', 'nccl', 'gloo'],
                        help='DDP backend; auto uses nccl when available, otherwise gloo')
    ...
    if opt.local_rank != -1:
        ...
        dist.init_process_group(backend=backend, init_method='env://')  # distributed backend
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0] and not opt.no_tensorboard:
