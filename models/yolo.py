...
class AFDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):
        ...
        self.register_buffer('anchors', torch.zeros(self.nl, 1, 2))
        self.register_buffer('anchor_grid', torch.zeros(self.nl, 1, 1, 1, 1, 2))
        self.m = nn.ModuleList(nn.Conv2d(x, self.no, 1) for x in ch[:self.nl])
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:
                if self.grid[i].shape[2:4] != p.shape[2:4]:
                    self.grid[i] = self._make_grid(nx, ny).to(p.device)
                y = p.sigmoid()
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]
                y[..., 2:4] = (y[..., 2:4] * 2.) ** 2 * self.stride[i] * 4.
                z.append(y.view(bs, -1, self.no))
...
class DualDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):
        super(DualDetect, self).__init__()
        self.nc = nc
        self.no = nc + 5
        self.nl = len(anchors)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            xi = x[i]
            for j, branch in enumerate((self.m[i], self.m2[i])):
                p = branch(xi)
                bs, _, ny, nx = p.shape
                p = p.view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
                out.append(p)
...
class DualAFDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):
        super(DualAFDetect, self).__init__()
        self.nc = nc
        self.no = nc + 5
        self.nl = len(anchors)
        self.na = len(anchors[0]) // 2
        ...
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch[:self.nl])
        self.af = nn.ModuleList(nn.Conv2d(x, self.no, 1) for x in ch[:self.nl])
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            p = self.af[i](xi)
            p = p.view(bs, 1, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            out.append(p)
            if not self.training:
                y = p.sigmoid()
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, AFDetect):
            s = 256  # 2x min stride
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
            self.stride = m.stride
            # print('Strides: %s' % m.stride.tolist())
        if isinstance(m, (DualDetect, DualAFDetect)):
            s = 256  # 2x min stride
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            c2 = ch[f] // 2
        elif m in [Detect, AFDetect, DualDetect, DualAFDetect, IDetect, IAuxDetect, IBin, IKeypoint]:
